...
from ._entropy_clm_configurations import ENTROPY_PCLM_CONFIG_ARGS


class LobsterEntropyPCLM(pl.LightningModule):
    def __init__(
        self,
        ...
        config_args = ENTROPY_PCLM_CONFIG_ARGS[model_name]
    ...
    def sequences_to_latents(self, sequences: list[str]) -> torch.Tensor:
        input_ids = self._transform_fn(sequences)['input_ids']
        with torch.inference_mode():
