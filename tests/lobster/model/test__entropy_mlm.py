...
from lobster.model import LobsterEntropyPMLM
...
def model():
    model = LobsterEntropyPMLM(model_name="MLM_mini", mask_percentage=0.1, initial_mask_percentage=0.8)
    model.eval()
    return model
...
@pytest.fixture(scope="module")
def esmc():
    model = LobsterEntropyPMLM(model_name="esmc")
    model.eval()
    return model


class TestLobsterEntropyPMLM:
